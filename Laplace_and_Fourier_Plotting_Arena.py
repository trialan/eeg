...
def get_coefficients(eigenvectors, subjects):
    ...
    label = get_data(subjects)[1] # recording label, ie 'hands' or 'feet' with 0,1
    coeffs0_vs_t_for_epoch = [] #coefficients through time corresponding to label 0
    coeffs1_vs_t_for_epoch = [] #...to label 1
    for i in range(len(label)):
        if (not label[i]):
            coeffs0_vs_t_for_epoch.append(ed.transform(get_data(subjects)[0][i].T).T)
        else: 
            coeffs1_vs_t_for_epoch.append(ed.transform(get_data(subjects)[0][i].T).T)
...
if __name__ == '__main__':
    ...
    #Coeffs0 = np.mean(coeffs0_vs_t_for_epoch, axis = 0)
    #Coeffs1 = np.mean(coeffs1_vs_t_for_epoch, axis = 0)
