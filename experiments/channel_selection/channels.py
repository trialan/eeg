...
from eeg.utils import results, get_cv, avg_power_vector, get_covariances, set_seed
...
def transform_to_eigenbasis(matrix, eigenvectors):
    ...
    for n_components in tqdm(list(range(10, 35))):
        ...
        X_ed = np.array(
            [
                transform_to_eigenbasis(
                    epoch, eigenvectors[:n_components, :n_components]
                )
                for epoch in channel_X
            ]
        )
        ...
        scores.append(score)
    plt.plot(
        list(range(10, 35)), scores, marker="o", linestyle="-", label="Laplacian+FgMDM"
    )
    plt.legend()
    plt.show()
